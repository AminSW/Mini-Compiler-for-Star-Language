    ...
    if (strcmp(command, "Keyword(write)") == 0) {
        ...
        while (*pos) {
            ...
            if (strncmp(pos, "String(\"", 8) == 0) {
                pos += 8; // Move past 'String("'
                while (*pos && *pos != '\"') {
                    putchar(*pos++);
                }
                if (*pos == '\"') pos++; // Move past closing quote
            } else if (strncmp(pos, "Identifier(", 11) == 0) {
                pos += 11; // Move past 'Identifier('
                char varName[32];
                int i = 0;
                while (*pos && *pos != ')') {
                    varName[i++] = *pos++;
                ...
                varName[i] = '\0';
                if (*pos == ')') pos++; // Move past closing parenthesis
                ...
                if (var != NULL) {
                    ...
                    printf("Error: Undeclared variable %s.", varName);
                }
            } else if (*pos == '.') {
                break;
            } else {
...
void handleLoop(int loopCount, const char* loopBody) {
    ...
    for (int i = 0; i < loopCount; i++) {
        ...
        while (token != NULL) {
            ...
            if (innerLoopActive) {
                if (strcmp(token, "LeftCurlyBracket") == 0) {
                    ...
                    handleLoop(innerLoopCount, innerLoopBody);
                    innerLoopBody[0] = '\0';
                    token = strtok(NULL, " ");
                    continue;
                }
            ...
            } else if (strcmp(token, "EndOfLine") == 0) {
                if (inStatement) {
                    //printf("Line: %s\n", statement);
                    execute(statement);
                    statement[0] = '\0';
            ...
            } else if (!innerLoopActive) {
                if (inStatement) {
                    strcat(statement, token);
                    strcat(statement, " ");
                } else if (strncmp(token, "Keyword", 7) == 0 || strncmp(token, "Identifier(", 11) == 0) {
                    inStatement = 1;
                    strcat(statement, token);
                    strcat(statement, " ");
                }
        ...
        // Her iterasyon sonunda döngü değişkenini artır
        strcpy(loopBodyCopy, loopBody);
        token = strtok(loopBodyCopy, " ");
        while (token != NULL) {
            if (strncmp(token, "Identifier(", 11) == 0) {
                char varName[50];
                sscanf(token, "Identifier(%[^)])", varName);
                Variable* var = getVariable(varName);
                if (var != NULL) {
                    token = strtok(NULL, " ");
                    if (token != NULL && strcmp(token, "Keyword(is)") == 0) {
                        token = strtok(NULL, " ");
                        if (token != NULL && strncmp(token, "Identifier(", 11) == 0) {
                            char srcVarName[50];
                            sscanf(token, "Identifier(%[^)])", srcVarName);
                            Variable* srcVar = getVariable(srcVarName);
                            if (srcVar != NULL) {
                                token = strtok(NULL, " ");
                                if (token != NULL && strcmp(token, "Operator(+)") == 0) {
                                    ...
                                    if (token != NULL && strncmp(token, "IntConst(", 9) == 0) {
                                        int value;
                                        sscanf(token, "IntConst(%d)", &value);
                                        var->intValue += value;
                                    } else if (strncmp(token, "Identifier(", 11) == 0) {
                                        sscanf(token, "Identifier(%[^)])", srcVarName);
                                        srcVar = getVariable(srcVarName);
                                        if (srcVar != NULL) {
                                            var->intValue += srcVar->intValue;
                                        } else {
...
void execute(const char* line) {
   // printf("Line: %s\n", line);
    ...
    if (strncmp(line, "Keyword(write)", 14) == 0) {
        ...
        // Keyword(loop) ifadesinin konumunu bul
        const char* loopStart = strstr(line, "Keyword(loop)");
        if (loopStart == NULL) {
            printf("Error: Keyword(loop) not found.\n");
            return; // Hata durumunda fonksiyondan çık
        }

        // IntConst ifadesini bul
        const char* loopCountStart = strstr(loopStart, "IntConst(");
        if (loopCountStart == NULL) {
            printf("Error: IntConst not found for loop count.\n");
            return; // Hata durumunda fonksiyondan çık
        }
        loopCountStart += strlen("IntConst("); // Sayıyı almak için ilgili pozisyona ilerle

        // loopCount değerini al
        int loopCount;
        sscanf(loopCountStart, "%d", &loopCount);

        // Döngü gövdesini almak için başlangıç konumunu bul
        const char* loopBodyStart = strstr(line, "Keyword(times)") + strlen("Keyword(times)");
        const char* loopBodyEnd = strchr(loopBodyStart, '\0'); // Döngü gövdesinin sonunu belirle
        char loopBody[4096];
